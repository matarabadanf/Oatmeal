...
from py_mods.src.SCF.scf_utils import (
    transformation_matrix, E_0_unrestricted_comp, guess_density, 
...
def CS_UHF(context: CS_UHF_ContextClass) -> CS_UHF_ResultsClass:
    ...
    dim = len(S)
    det_alpha, det_beta, _ = validate_unrestricted_determinant( n_electrons, occupation, dim, mult)

    alpha_elec = sum(det_alpha)
    # n_alpha_initial = alpha_elec
    # alpha_occ = det_alpha
    beta_elec = sum(det_beta)
    # n_beta_initial = beta_elec
    # beta_occ = det_beta
    ...
    if p_guess == 'RHF':
        ...
        if isinstance(guess_MAX_ITER, int):
            ...
            guess_iter = 12
            guess_context = CS_RHF_ContextClass(S.real, T.real, V.real, eri.real, n_electrons=elec_pre, theta=0, max_iter=guess_iter, threshold=1E-14, p_guess='core', verbose=False)
        ...
        P_LR_alph = np.copy(INPORB) 

    else: 
        P_LR_alph = guess_density(dim, p_guess)
    
    # P_LR_alph *= S # this leads to a closer guess to the PySCF one
    P_LR_beta = np.copy(P_LR_alph)
    
    if break_symm: #note that breaking symmetry will only make sense when the guess is not zeros
        P_LR_beta[:n_electrons, :n_electrons] = 0
    ...
    for iteration in range(max_iter):
        ...
        F_alph, r_alph, F_beta, r_beta = calculate_unrestricted_F_and_r_comp(P_LR_alph, P_LR_beta, S, H_core, eri_scaled)
        ...
        if iteration > 5 and error < threshold:
            ...
            P_LR_alph, C_munu_alph, e_alph, R_alph, L_alph, P_RR_alph, C_prime_alph = calculate_P_next(F_next_alph, X, alpha_elec, det_alpha, mode='UHF')
            P_LR_beta, C_munu_beta, e_beta, R_beta, L_beta, P_RR_beta, C_prime_beta = calculate_P_next(F_next_beta, X,  beta_elec, det_beta,  mode='UHF')
            break
        ...
        if not use_conv:
            ...
            try:
                F_opt_alph, r_opt_alpha = calc_diis_extrapolation(residuals_alph, F_guess_alph)
                F_opt_beta, r_opt_beta  = calc_diis_extrapolation(residuals_beta, F_guess_beta)
        ...
        if theta == 0.0:
            P_LR_alph = P_LR_alph.real.astype(np.complex128)
            L_alph = L_alph.real.astype(np.complex128)
    ...
    S_diagnostics = calculate_s2_expectation(P_LR_alph, P_LR_beta, S, verbose)

    n_alpha = np.trace(P_LR_alph.real @ S)
    n_beta  = np.trace(P_LR_beta.real @ S)

    assert abs(n_alpha + n_beta - n_electrons) < 1E-10, 'Number of electrons was not conserved in the calculation'

    ResultClass = CS_UHF_ResultsClass(context, converged, E_UHF, e_alph, e_beta, n_alpha, n_beta, X, P_guess_alpha, P_guess_beta, P_LR_alph, P_LR_beta, P_total, P_diff, L_alph, R_alph, L_beta, R_beta, S_diagnostics, error, iteration)
