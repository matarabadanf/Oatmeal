...
def obara_saika_bottom_up(
    Ax: float, Bx: float, a: float, b: float, i: int, j: int
) -> NDArray[np.float64]:
    """
    ...
    for total in range(1, max_dim):
        for i in range(total, max_dim):
            S_ij_mat[i, total] = X_pa * S_ij_mat[i - 1, total] + 1 / (2 * p) * (
                (i - 1) * S_ij_mat[i - 2, total] + (total) * S_ij_mat[i - 1, total - 1]
            )
        for j in range(total, max_dim):
            S_ij_mat[total, j] = X_pb * S_ij_mat[total, j - 1] + 1 / (2 * p) * (
                (total) * S_ij_mat[total - 1, j - 1] + (j - 1) * S_ij_mat[total, j - 2]
            )
