...
def test_H2() -> None:
    ...
    E_hf = E_elec + nuc_nuc
    assert (
        abs(E_hf - E_hf_sto3g_H2) < 1e-6
    ), f"SCF energy does not match reference value {E_hf} != {E_hf_sto3g_H2}"
...
def test_He() -> None:
    ...
    assert converged, "Calculation did not converge"
    assert (
        abs(E_hf - E_hf_ccpvdz_He) < 1e-8
    ), f"SCF energy does not match reference value {E_hf} != {E_hf_ccpvdz_He}"
...
def test_Be() -> None:
    ...
    assert converged, "Calculation did not converge"
    assert (
        abs(E_hf - E_hf_ccpvdz_Be) < 1e-8
    ), f"SCF energy does not match reference value {E_hf} != {E_hf_ccpvdz_Be}"
...
def test_DIIS() -> None:
    ...
    assert converged, "Calculation did not converge"
    assert (
        abs(E_RHF - E_hf_augccpvqz_HF) < 1e-8
    ), f"SCF energy does not match reference value {E_RHF} != {E_hf_augccpvqz_HF}"
