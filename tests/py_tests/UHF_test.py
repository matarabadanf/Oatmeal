...
def test_B_huge() -> None:
    ...
    assert abs(CS_UHF_results.E_UHF.real - E_hf_aug_cc_pvqz_B) < 1E-8, f"SCF energy does not match reference value {CS_UHF_results.E_UHF.real} != {E_hf_aug_cc_pvqz_B}"
...
def test_N() -> None:
    ...
    CS_UHF_results = CS_UHF(context)
    assert CS_UHF_results.converged, "Calculation did not converge"
    assert abs(CS_UHF_results.E_UHF.real - E_hf_aug_cc_pvqz_N) < 1E-8, f"SCF energy does not match reference value {CS_UHF_results.E_UHF.real} != {E_hf_aug_cc_pvqz_N}"
...
def test_H2_dissociation():
    ...
    reference_E = np.array(
        [-2.37960889, -1.43448906, -1.22282276, -1.15386862, -1.11765807,
    ...
    for i, dist in enumerate(distances):
        ...
        context = CS_UHF_ContextClass(overlap, kin, vnuc, eri, n_electrons=n_elec, p_guess='RHF', break_symm=True, max_iter=200)
        CS_UHF_results = CS_UHF(context)
