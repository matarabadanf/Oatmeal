...
def test_theta_zero() -> None:
    ...
    converged, E_hf, E_e_values, C_munu, P = RHF(
        S_sto3g_li,
        T_sto3g_li,
        V_sto3g_li,
        eri_sto3g_li,
    ...
    assert (
        abs(Li_results.E_RHF - E_hf_sto3g_li) < 1e-8
    ), f"CS-RHF energy does not match unscaled reference value {Li_results.E_RHF} != {E_hf_sto3g_li}"
...
def test_theta_non_scaled() -> None:
    ...
    assert (
        abs(even_s29_res.E_RHF - E_hf_29s_He) < 1e-8
    ), f"CS-RHF energy does not match unscaled reference value {even_s29_res.E_RHF} != {E_hf_29s_He}"
...
def test_theta_18_scaled() -> None:
    ...
    assert (
        abs(He_29s_res.E_RHF - E_hf_29s_He) < 1e-8
    ), f"CS-RHF energy does not match unscaled reference value {He_29s_res.E_RHF} != {E_hf_29s_He}"
...
def test_theta_excited_non_scaled() -> None:
    ...
    assert (
        abs(He_29s_2s2_results.E_RHF - E_hf_29s_He) < 1e-8
    ), f"CS-RHF energy does not match unscaled reference value {He_29s_2s2_results.E_RHF} != {E_hf_29s_He}"
...
def test_theta_excited_non_scaled_huge_basis() -> None:
    ...
    assert He_29s_2s2_results.converged == True, "CS-RHF Calculation did not converge"
    assert (
        abs(He_29s_2s2_results.E_RHF - E_hf_aug_5Z_He) < 1e-8
    ), f"CS-RHF energy does not match unscaled reference value {He_29s_2s2_results.E_RHF} != {E_hf_aug_5Z_He}"
...
def test_qchem_21s() -> None:
    ...
    assert (
        abs(np.mean(traj_energies[1] - k)) < 1e-8 + 1e-8j
    ), f"Mean error is too large in 1s2 UHF: {abs(np.mean(traj_energies[1]-k))}"
    ...
    assert (
        abs(np.mean(traj_energies[1] - k2)) < 1e-8 + 1e-8j
    ), f"Mean error is too large in 2s2 UHF: {abs(np.mean(traj_energies[1]-k))}"
