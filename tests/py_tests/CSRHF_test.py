...
def test_theta_excited_non_scaled_huge_basis() -> None:
    ...
    occupation_determinant = np.array([0,2,0])
    converged, E_elec_comp, E_e_values, C_munu, P = CS_RHF(S_aug_5Z_He, T_aug_5Z_He, V_aug_5Z_He, eri_aug_5Z_He, 2, theta=0.0, occupation=occupation_determinant, max_iter=500, threshold=1E-12, p_guess='core', verbose=False)
    assert converged == True, "CS-RHF Calculation did not converge"
